...
var MUSIC=(function(){
 ...
 function build(){
  ...
  /* سمّاعة الهاتف غشاءٌ صغير لا يُصدر ما دون ~٣٠٠ هرتز. واللحن ينزل إلى ٤١،
     ...
     فمرشّحان متتاليان عند ١٩٠ هرتز (٢٤ ديسيبل/أوكتاف): عند ٩٥ هرتز ‎-٢٤‎
     وعند ٤١ نحو ‎-٤٠‎ — فلا يصل السمّاعةَ ما تعجز عنه. */
  var hp1=ctx.createBiquadFilter(),hp2=ctx.createBiquadFilter();
  hp1.type=hp2.type='highpass';
  hp1.frequency.setValueAtTime(190,ctx.currentTime);
  hp2.frequency.setValueAtTime(190,ctx.currentTime);
  hp1.Q.setValueAtTime(0.7,ctx.currentTime);
  hp2.Q.setValueAtTime(0.7,ctx.currentTime);
  ...
  master.connect(hp1);hp1.connect(hp2);hp2.connect(lim);lim.connect(ctx.destination);
  return ctx;
 }
