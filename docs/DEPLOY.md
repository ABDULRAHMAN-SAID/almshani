# نشر تحدّي — التطبيق والويب والخادم والمتاجر (v5.97)
...
**الدخول** (5.95) يعمل بلا أيّ إعداد: بريد وضغطة. وما يلي اختياريّ يزيده قوّة.

**زرّ Google** (دخول بضغطة واحدة على كل جهاز، بلا رمز أبدًا): من
[console.cloud.google.com](https://console.cloud.google.com/apis/credentials) أنشئ
**OAuth client ID** نوعه *Web application*، وأضف نطاقك في
*Authorized JavaScript origins*، ثم:

```bash
fly secrets set TAHADDI_GOOGLE_CLIENT_ID=xxxx.apps.googleusercontent.com
```

(هذا المعرّف علنيّ بطبيعته ويظهر في الصفحة — والسرّ المقابل له لا يُستعمل هنا أصلًا.)

**مزوّد بريد** (للحالة الوحيدة التي تحتاج رمزًا: بريد يملكه حساب آخر). من
...
| `TAHADDI_MAIL_URL` / `_KEY` / `_FROM` | فارغ = معطّل | مزوّد البريد. **لم يعد شرطًا للدخول** منذ 5.95: الدخول بالبريد بلا رمز. يلزم فقط حين يدخل لاعب ببريد يملكه حساب آخر — وبدونه يبقى طريق «نقل الحساب برمز» يعمل |
| `TAHADDI_GOOGLE_CLIENT_ID` | فارغ = مخفيّ | معرّف عميل Google OAuth (نوع Web). **ليس سرًّا** — يُنشر في الصفحة عمدًا، والخادم وحده يتحقّق من الرمز مع Google. بضبطه يظهر زرّ «المتابعة بحساب Google» في شاشة الدخول |
...
## ٣. تطبيق أندرويد — ملفّ APK يُثبَّت على الهاتف (5.97)

هذا أقصر طريق إلى «لعبة على هاتفي»: لا حساب، ولا متجر، ولا خادم، ولا بطاقة.
البناء يجري على خوادم GitHub، وتنزّل الملفّ وتثبّته.

**كيف تأخذ النسخة:**

1. في المستودع: **Actions** → «تطبيق أندرويد (APK)» → **Run workflow**.
   (وكلّ دفعة تمسّ `tahaddi/` أو `app-android/` تشغّله تلقائيًّا أيضًا.)
2. انتظر انتهاء التشغيل، ثمّ افتح صفحته ونزّل من **Artifacts**: `tahaddi-apk-<الإصدار>`.
3. فُكّ ضغط الملفّ المنزَّل، وانقل ملفّ `.apk` إلى هاتفك، وافتحه.
   أندرويد سيسأل «السماح بالتثبيت من هذا المصدر؟» — اسمح مرّة واحدة.

**ما داخل التطبيق:** اللعبة نفسها بالحرف — `tools/android-sync.cjs` ينسخ ناتج
`tools/build-www.cjs` إلى `app-android/app/src/main/assets/`، فلا يوجد مسار بناء ثانٍ يتأخّر
عن الأوّل، ورقم الإصدار يُقرأ من `APP_VER` داخل اللعبة لا يُكتب مرّتين.

**قرارات تقنية لها سبب:**

- الصفحة تُحمَّل من `https://appassets.androidplatform.net/assets/index.html` عبر
  `WebViewAssetLoader`، لا من `file://`. السبب: `file://` ليس سياقًا آمنًا فلا يوجد
  `crypto.subtle`، وختم الغرف (ECDH + AES-GCM) يرفض العمل، فتسقط الغرف كلّها. النطاق
  محجوز من أندرويد ولا يُحلّ على الإنترنت: كلّ طلب يُقرأ من داخل الحزمة.
- `setMediaPlaybackRequiresUserGesture(false)` كي تبدأ الموسيقى مع فتح التطبيق.
- زرّ الرجوع في الهاتف يستدعي `webView.goBack()`، فيصل إلى الموجّه كـ`popstate`
  ويرجع شاشة واحدة — بلا جسر جافاسكربت.
- حواف النافذة: التطبيق يُبعد اللعبة عن الشريطين ولوحة المفاتيح بـ`WindowInsets`،
  لا بـ`env(safe-area-inset-*)` التي لا يملؤها WebView على كلّ الأجهزة.
- الخطّ مضمَّن داخل اللعبة (`tools/build-fonts.cjs`)، فالتطبيق يعرض الخطّ الصحيح من أوّل
  فتح وبلا إنترنت، ولا يخرج منه طلب واحد إلى الشبكة ما لم يُضبط خادم.
- عامل الخدمة لا يُسجَّل داخل التطبيق: الأصول في الحزمة أصلًا، وتخزينها ثانية يضيع
  عشرة ميجابايت من الهاتف.

**بالخادم:** اضبط متغيّر المستودع `TAHADDI_SERVER` (Settings → Secrets and variables →
Actions → Variables) إلى `https://<app>.fly.dev` قبل التشغيل، أو مرّره في خانة
«عنوان الخادم» عند Run workflow — يُكتب في `<meta name="tahaddi-server">` فتشتغل
الحسابات والغرف واللوحة. وعلى الخادم أضف `https://appassets.androidplatform.net`
إلى `TAHADDI_ORIGINS` إن كنت تقيّد الأصول.

**قبل الفحص:** `NODE_PATH=$(npm root -g) node tools/test-android-shell.cjs` يشغّل اللعبة
على أصل الحزمة نفسه في متصفّح حقيقي ويتحقّق ممّا لا يظهر إلا داخل التطبيق: السياق آمن،
`crypto.subtle` موجود، لا خطأ إقلاع، لا طلب يخرج، الخطّ من الحزمة، لا عامل خدمة.

هذه الحزمة موقَّعة بمفتاح التصحيح: تُثبَّت باليد ولا تُرفع إلى Play. للمتجر انظر §٥.

## ٤. PWA — التثبيت والعمل دون اتصال
...
## ٧. ما لا يستطيع هذا المستودع فعله وحده
...
- لا عدّة أندرويد في هذه البيئة: حزمة APK تُبنى على خوادم GitHub (§٣)، لا هنا. وiOS يحتاج Xcode على جهاز Mac.
- لا نطاق: TWA يحتاج نطاقًا بـ `https` تملكه (GitHub Pages يعطيك `<user>.github.io`).

## ٨. قائمة التحقّق قبل الإطلاق
...
6. `NODE_PATH=$(npm root -g) node tools/test-android-shell.cjs` أخضر، ثمّ شغّل «تطبيق أندرويد (APK)»
   ونزّل الحزمة وثبّتها على هاتف: تفتح، تشتغل الموسيقى، زرّ الرجوع يرجع شاشة واحدة.
