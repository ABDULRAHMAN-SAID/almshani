...
function render(){
 ...
 document.getElementById('app').innerHTML=
  '<div class="wrap">'
  +'<h1>خلفيات <span>الأندية</span></h1>'
  +'<p class="sub">ارفع صورة لكل حيوان: صقر · أسد · ذئب · نمر · حصان · جمل — أو أي حيوان تريده. '+'اكتب في الملاحظات اسم كل صورة بالترتيب، وأنا أزرعها في اللعبة كخلفيات يختارها القائد عند إنشاء النادي.</p>'
  +'<div class="card">'
...
def build(state):
    ...
    head = ('<title>خلفيات أندية تحدي</title>\n<style>' + CSS + '</style>\n'
            + '<script id="st" type="application/json">' + j + '</script>\n'
            + '<div id="app"></div>\n'
            + '<script>var SRC=' + json.dumps(src) + ';\n' + src + '\n</script>\n')
    return head

open(SP+'tahaddi-clanart.html','w',encoding='utf-8').write(build({"imgs":[],"note":""}))
print('built', os.path.getsize(SP+'tahaddi-clanart.html'))
