...
/* ═══ 3.5) أسئلة القوانين الحسابية (فيزياء الواجبات) — تُحذف: هذه لعبة لا امتحان ═══ */
let nPhys = 0;
Q = Q.filter(q => {
  if (/قانون أوم|فرق الجهد|أمبير.*أوم|القوة.*نيوتن|الطاقة الحركية|وفق قانون/.test(q.t)) { nPhys++; return false }
  return true;
});
console.log('أسئلة قوانين حسابية حُذفت:', nPhys);
