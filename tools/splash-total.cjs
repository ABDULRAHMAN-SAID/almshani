...
/* ٦٫٩٥ — شاشة الدخول تقيس ما نزل من الملفّ فعلًا، فتحتاج حجمه الكلّيّ وموضع السكربت الرئيسيّ: يُعاد حسابهما بعد كلّ تعديل
   (عرضٌ ثابت من عشرة أرقام حتّى لا يغيّر الحساب الحجم). node tools/splash-total.cjs [--check] */
...
const b=Buffer.from(s,'utf8');const total=b.length,head=b.indexOf('<script id="mainjs">');
const cur=[+m[1],+m[2]];const want=[total,head];
if(cur[0]===want[0]&&cur[1]===want[1]){console.log('✓ أرقام شاشة الدخول مضبوطة — '+total+' بايت');process.exit(0)}
...
fs.writeFileSync(P,s);console.log('✓ حُدّثت أرقام شاشة الدخول: '+total+' بايت، السكربت عند '+head);
