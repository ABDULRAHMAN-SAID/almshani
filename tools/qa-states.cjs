...
module.exports=[
 ...
 ['rmInvite',R("tab('play');RM.code='ABCD';RM.game='mafia';RM.host=1;RM.solo=0;push('roomScr','mafia');setTimeout(()=>{try{rmInvite()}catch(e){}},300)")],
 ['knBot',R("push('rankedScr');knBot(58)")],
 ['playModesScr',R("push('playModesScr')")],
 ['partyScr',R("push('partyScr')")],
 ...['carrom','uno','draw','knowledge'].map(g=>[`gameHub-${g}`,R(`push('gameHub','${g}')`)]),
 ...['knowledge','carrom','uno','drawing'].map(g=>[`gameRankScr-${g}`,R(`push('gameRankScr','${g}')`)]),
 ...
 ['lk-back',R("push('partyScr');push('lkSetup');PT.n=4;lkStart()")],
 ['lk-face-two',R("push('partyScr');push('lkSetup');PT.n=4;lkStart();PT.kind='real';PT.card=LK_CARDS.real[0];PT.flip=1;lkDraw()")],
 ['lk-face-dare',R("push('partyScr');push('lkSetup');PT.n=4;lkStart();PT.kind='dare';PT.card=LK_CARDS.dare[0];PT.flip=1;lkDraw()")],
 ['lk-end',R("push('partyScr');push('lkSetup');PT.n=4;lkStart();PT.pts=[3,5,2,5];lkEnd()")],
 // ── ضد الكمبيوتر: مافيا وبرا السالفة والرسم ──
 ...['mafia','draw'].map(g=>[`soloScr-${g}`,R(`push('gameHub','${g}');push('soloScr','${g}')`)]),
