...
(async()=>{
 ...
 const mk=async label=>{
  ...
  await page.evaluate(()=>{S.tutorial_completed=true;S.tutDone=1;S.dev=1;saveState()});
 ...
 try{
  ...
  await C.evaluate(tok=>{S.tutorial_completed=true;S.account={token:tok};S._savedAt=0;saveState();netBoot()},acc.token);
