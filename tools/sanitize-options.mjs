...
// ── 3) «أي السورتين …»: السورتان المذكورتان فقط ──
for (const q of Q) {
  if (!q.o || q.tf || q.fill || q._drop) continue;
  if (!/^أي السورتين/.test(q.t)) continue;
  const m = q.t.match(/سورة\s+(.+?)\s+أم\s+سورة\s+([^؟?]+)/);
  if (!m) { drop(q); continue }
  const A = m[1].trim(), B = m[2].trim();
  const corr = String(q.o[q.c] ?? '');
  let pick = null;
...
for (const [key, list] of Object.entries(fam)) {
  ...
  if (/^(ما نتيجة|من فاز|أي السورتين|ما العدد|متتالية)/.test(key)) continue; // عولجت أعلاه
